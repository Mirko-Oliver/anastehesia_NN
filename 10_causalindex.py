"""
Build a Causalindex, using a Generalized Additive Model 
"""
...
# More flexible only for SR + EEG bands
NODE_N_SPLINES_MAIN = {
    "BIS/SR": 10,
    "EEG1_alpha_rel": 10,
    "EEG1_beta_rel": 10,
...
# lower complexity for modifiers unless you explicitly want otherwise
NODE_N_SPLINES_MOD = {
    "age": 4,
    "sex": 4,
    "Solar8000/BT": 4,
...
def allocate_global_bucket_quotas(
    ...
    *,
    ...
    """
    Allocate a strict global cap per BIS bucket across all cases.
    ...
    Returns:
        case_quota_map[cid] = int array of shape (N_BINS,)
    """
...
if __name__ == "__main__":
    ...
    print("[INFO] Running lambda grid search on subset...")
    gam = LinearGAM(TERMS)

    n_lams = len(flatten(gam.lam))
    n_draws = 200
    lam_low, lam_high = 1e-7, 1e2
    ...
    best_lambda = gam.lam
    print("[INFO] Best lambda found:", best_lambda)

    gam_final = LinearGAM(TERMS, lam=best_lambda)
    ...
    joblib.dump(bundle, "bis_causal_gam.pkl")
    print("[INFO] Model bundle saved")

    y_val_pred = gam_final.predict(X_val)
    y_test_pred = gam_final.predict(X_test)

    print_metrics("VAL", y_val, y_val_pred)
    print_metrics("TEST", y_test, y_test_pred)

    print_rmse_by_bis_bin("VAL", y_val, y_val_pred, BIS_EDGES)
    print_rmse_by_bis_bin("TEST", y_test, y_test_pred, BIS_EDGES)
